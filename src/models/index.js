...
const userSchema = new mongoose.Schema({
  ...
  /*
   * email — legacy field kept for backward compat with old login queries.
   * sparse:true means MongoDB skips null/undefined values from the unique index,
   * so multiple users without an email field don't collide.
   * DO NOT add userSchema.index({ email:1 }) separately — unique:true already creates it.
   */
  email:              { type: String, unique: true, sparse: true, lowercase: true, trim: true },

  /*
   * officialEmail — primary login credential for company-registered users.
   * sparse:true for the same reason as email above.
   * DO NOT add userSchema.index({ officialEmail:1 }) separately.
   */
  ...
  /* ── Registration documents (up to 5) ── */
...
/*
 * Explicit compound / non-unique indexes only.
 * Do NOT repeat indexes that are already created by unique:true on the field definition
 * (email, officialEmail) — that causes the Mongoose duplicate-index warning.
 */
...
const adminSchema = new mongoose.Schema({
  name:        { type: String, required: true, trim: true },
  // unique:true on email already creates the index — no adminSchema.index({ email:1 }) needed
...
const portSchema = new mongoose.Schema({
  // unique:true on code already creates the index — no portSchema.index({ code:1 }) needed
...
// Only non-unique compound/text indexes here
...
const bookingSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['pending','under_review','approved','rejected','confirmed','cancelled'],
    default: 'pending',
  },
  reviewedBy:  { type: mongoose.Schema.Types.ObjectId, ref: 'Admin' },
  reviewedAt:  Date,
...
bookingSchema.pre('save', function (next) {
  if (!this.bookingRef)
    this.bookingRef = `NGR-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
  next();
});
...
const enquirySchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['pending','under_review','responded','closed'],
    default: 'pending',
  },
